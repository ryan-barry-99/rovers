...
from subsystems.cameras.ScienceCamera import ScienceCamera  # Importing the ScienceCamera class from the specified module
from RoverConstants import LIPID_DETECTED, LIPID_NOT_DETECTED, DNA_DETECTED, DNA_NOT_DETECTED  # Importing constants
...
class SciencePlate(Node):  # Defining a class named SciencePlate that inherits from the Node class
    def __init__(self):  # Constructor for the SciencePlate class
        super().__init__("science_plate")  # Initializing the parent class Node with the name "science_plate"
        self.camera = ScienceCamera()  # Creating an instance of the ScienceCamera class and assigning it to the camera attribute
        self.lipid_publisher = self.create_publisher(Int32, 'science/lipid_detection', 10)  # Creating a publisher for lipid detection messages
        self.dna_publisher = self.create_publisher(Int32, 'science/dna_detection', 10)  # Creating a publisher for DNA detection messages
    ...
    def publish_lipid_detection(self):  # Method for publishing lipid detection status
        ...
        lipid_detection_msg.data = self.lipid_detection  # Setting the data field of the message to the value of lipid_detection
    ...
    def publish_dna_detection(self):  # Method for publishing DNA detection status
        ...
        dna_detection_msg.data = self.dna_detection  # Setting the data field of the message to the value of dna_detection
